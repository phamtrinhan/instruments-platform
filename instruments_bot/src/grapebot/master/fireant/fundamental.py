...
def task_transform(mydata_ii, stock_ii):
    mapping_type = {
        1: {
    ...
    if (mydata_ii == None):
        ...
        logger.warn(
            f"Not found on {stock_ii['stock']} on {mapping_type[int(stock_ii['type'])]}")
        return []
    ...
    for section in tmp:
        ...
        for single in section['values']:
            if single['quarter'] == 0:
                query = tsm[(tsm.quarter == 5) & (tsm.year == single['year']) & (
                    tsm.stock == stock_ii['stock'])]
                ...
                query = tsm[(tsm.quarter == single['quarter']) & (
                    tsm.year == single['year']) & (tsm.stock == stock_ii['stock'])]
...
def get_all_info(stock_list=None, type=None, end=None):
    ...
    logger.info('-' * 10)
    logger.info(
        f'Start get FIREANT FUNDAMENTAL {type.upper()}: #{count[type.lower()]}')
    if (stock_list is None) or (type is None):
        logger.error("FIREANT FUNDAMENTAL EMPTY !!!!")
        return
    DATA_PATH_CSV = storage_utils.create_daily_file(
        f"/notion/HIST_FIREANT_FUNDAMENTAL/CSV/")
    DATA_PATH_JSON = storage_utils.create_daily_file(
        f"/notion/HIST_FIREANT_FUNDAMENTAL/JSON/")
    DATA_PATH_PICKLE = storage_utils.create_daily_file(
        f"/notion/HIST_FIREANT_FUNDAMENTAL/PICKLE/")
    ...
    report_type_data = loop.run_until_complete(
        getBase.getByList_async(get_link, body_list, [], 0.1, True))
    ...
    for i in range(len(report_type_data)):
        try:
            ...
            print(report_type_data[i])
            logger.error(e)
            logger.error(stock_list[i]['code'])
            current = []
    bfinal = []

    with Pool() as pool:
        # perform calculations
        results = pool.starmap(task_transform, zip(mydata, stock_dict))
    for s_final in results:
        bfinal += s_final

    tmp_data = pd.DataFrame(bfinal)
